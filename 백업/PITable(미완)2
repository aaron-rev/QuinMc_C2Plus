...
int CheckNumberofOne(int* PITableRow, int TMCount);
...
int main()
{
	...
	while (!CheckBreak(PITable[PICount], TMCount)) //확인행이 모두 1이 될 때까지 반복
	{
		...
		for (int i = 0; i < PICount; i++)
		{
			if (CheckNumberofOne(PITable[i], TMCount) > Max) MaxIdx = i;
		}

		for (int i = 0; i < len; i++)
		{
			EssentialPI[EssentialPICount][i] = PIColumn[MaxIdx][i];
		}
		EssentialPICount++;

		/* 거기에 해당되는 1들을 확인행에 체크 */
		for (int i = 0; i < TMCount; i++)
		{
			if (PITable[MaxIdx][i] == 1)
			{
				PITable[PICount][i] = 1;
			}
		}

		/* 확인행을 살펴 본 후 1이 아닌 요소가 해당되는 열에 1이 있는지 확인 */
	}
