...
int main()
{
	...
	while (!CheckBreak(PITable[PICount], TMCount)) //확인행이 모두 1이 될 때까지 반복
	{
		...
		/* 1이 있다면 그 행에 다른 1이 있는지 확인
		   다른 1이 있는 경우 EPI에 PI를 추가하고 1의 위치를 파악해서 밑에 확인행에 1 체크
		( 단, 이미 1이 체크되어 있는 경우 체크해서는 안됨 / 반복문을 빠져나가는 조건이 확인행을 모두 더했을 때의 합이 행의 개수가 나오는 것이기 때문) */
